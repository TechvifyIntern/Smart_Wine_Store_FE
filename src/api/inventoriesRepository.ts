...
class InventoriesRepository extends BaseRepository {
  ...
  async getInventoryByProduct(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Unknown error";
      throw new Error(
        `Failed to get inventory for product ${productId}: ${message}`
      );
    }
  }
  ...
  async getInventoryByWarehouseAndProduct(
    ...
    try {
      const response = await api.get(
        `${this.endpoint}/warehouse/${warehouseId}/product/${productId}`
      );
      return response.data;
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unknown error";
      throw new Error(
        `Failed to get inventory for warehouse ${warehouseId} and product ${productId}: ${message}`
      );
    }
  }
  ...
  async importInventory(
    importData: ImportInventoryData
  ...
  async exportInventory(
    exportData: ExportInventoryData
  ): Promise<ApiResponse<InventoryLog>> {
    try {
   ...
   */
  async batchImportInventory(
    imports: ImportInventoryData[]
  ): Promise<ApiResponse<InventoryLog[]>> {
    try {
      const response = await api.post(`${this.endpoint}/import/batch`, {
        imports,
      });
      return response.data;
    } catch (error) {
   ...
   */
  async batchExportInventory(
    exports: ExportInventoryData[]
  ): Promise<ApiResponse<InventoryLog[]>> {
    try {
      const response = await api.post(`${this.endpoint}/export/batch`, {
        exports,
      });
      return response.data;
    } catch (error) {
