  ...
  return (
    ...
    >
      ...
      <td className="px-6 py-4">
        <div className="dark:text-slate-400 text-sm">{account.Email}</div>
      </td>
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="dark:text-slate-400 text-sm">{account.PhoneNumber}</div>
      </td>
      <td className="px-6 py-4 whitespace-nowrap">
        <span
          className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${getRoleBadgeClass(account.RoleName)}`}
        >
          {account.RoleName}
        </span>
      </td>
      <td className="px-6 py-4 whitespace-nowrap">
        <span
          className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium transition-all duration-200 ${getTierBadgeClass(account.TierName)}`}
        >
          {account.TierName}
        </span>
      </td>
      <td className="px-6 py-4 whitespace-nowrap text-center">
        <span
          className={`text-sm font-semibold dark:text-slate-300 ${
            isLoyalCustomer
              ? "group-hover:text-yellow-600 dark:group-hover:text-yellow-400 transition-colors duration-200"
              : ""
          }`}
        >
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <div onClick={(e) => e.stopPropagation()}>
          <Select
            ...
            onValueChange={(value) =>
              onStatusChange(account.UserID, parseInt(value))
            }
          >
            <SelectTrigger
              className={`w-[110px] h-7 text-xs font-medium border-0 ${getStatusBadgeClass(account.StatusName)}`}
            >
              <SelectValue />
            </SelectTrigger>
