...
import { updateInventoryProductSchema, UpdateInventoryProductFormData } from "@/validations/inventories/inventoryProductSchema";
import { Upload, X, Image as ImageIcon } from "lucide-react";

export interface UpdateInventoryProductProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    product: InventoryProduct | null;
    onUpdate?: (productId: string, data: Omit<InventoryProduct, "ProductID">) => void | Promise<void>;
}
...
export function UpdateInventoryProduct({
    ...
    useEffect(() => {
        if (product && open) {
            ...
            setUploadMethod(product.ImageURL.startsWith("data:image/") ? "upload" : "url");
        } else if (!open) {
            reset();
            setImagePreview("");
            setUploadMethod("url");
    ...
    const onSubmit = async (data: UpdateInventoryProductFormData) => {
        if (!product) return;

        try {
            if (onUpdate) {
                await onUpdate(product.ProductID, data);
            }
            onOpenChange(false);
            reset();
        } catch (error) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto bg-white! border-gray-200!">
                <DialogHeader>
                    <DialogTitle className="text-gray-900!">Update Product</DialogTitle>
                    <DialogDescription className="text-gray-600!">
                        Update the details for {product.ProductName} (ID: {product.ProductID})
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-gray-500">Use Import/Export actions for stock adjustments</p>
                        </div>
