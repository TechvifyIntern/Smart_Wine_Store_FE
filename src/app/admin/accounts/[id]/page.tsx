...
export default function AccountDetailPage() {
  ...
  useEffect(() => {
    const fetchAccount = async () => {
      try {
        ...
        setError(null);
        const accountData =
          await userManagementRepository.getUserById(accountId);
  ...
  const handleStatusChangeConfirm = async () => {
    ...
    try {
      ...
      const reason =
        newStatusId === 2 ? "Violation of terms and conditions" : undefined;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center gap-3 p-4 bg-white/60 dark:bg-slate-700/60 rounded-lg border border-white/80 dark:border-slate-600">
                  ...
                  <div>
                    ...
                    <span
                      className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${getTierColor(account.TierName)}`}
                    >
                      {account.TierName}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-3 p-4 bg-white/60 dark:bg-slate-700/60 rounded-lg border border-white/80 dark:border-slate-600">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-slate-100">
                      {account.MinimumPoint.toLocaleString()}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-4 bg-white/60 dark:bg-slate-700/60 rounded-lg border border-white/80 dark:border-slate-600">
                  ...
                  <div>
                    ...
                    <span
                      className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(account.StatusName)}`}
                    >
                      {account.StatusName}
                    </span>
                  </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription
              ...
              dangerouslySetInnerHTML={{
                __html: getStatusChangeDialogDescription(),
              }}
            />
          </AlertDialogHeader>
          <AlertDialogFooter>
