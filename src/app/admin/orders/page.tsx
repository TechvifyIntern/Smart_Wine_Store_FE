...
export default function OrdersManagementPage() {
    ...
    const handleExportPDFs = () => {
        const ordersToExport = selectedOrders.length > 0 ? selectedOrders : orders.map(o => o.OrderID);
    ...
    const exportSingleOrderPDF = (order: Order) => {
        ...
        const addCenteredText = (text: string, fontSize: number, y: number, color: number[] = darkText) => {
        ...
        const addLeftRightText = (leftText: string, rightText: string, y: number, fontSize: number = 10, color: number[] = darkText) => {
            doc.setFontSize(fontSize);
            doc.setTextColor(color[0], color[1], color[2]);
        ...
        doc.setFontSize(28);
        doc.text("🍷 SMART WINE STORE", pageWidth / 2, yPosition, { align: "center" });
        yPosition += 10;

        doc.setFont("helvetica", "normal");
        doc.setFontSize(14);
        doc.setTextColor(100, 100, 100);
        doc.text("Premium Wine Collection & Events", pageWidth / 2, yPosition, { align: "center" });
        yPosition += 8;

        doc.setFontSize(10);
        doc.setTextColor(120, 120, 120);
        doc.text("123 Wine Street, Vineyard City, VC 12345", pageWidth / 2, yPosition, { align: "center" });
        yPosition += 5;
        doc.text("Phone: (555) 123-4567 | Email: info@smartwinestore.com", pageWidth / 2, yPosition, { align: "center" });
        yPosition += 15;
        ...
        doc.setFont("helvetica", "bold");
        doc.setFontSize(16);
        doc.text(`INVOICE #${order.OrderID}`, pageWidth / 2, yPosition + 2, { align: "center" });
        yPosition += 25;
        ...
        yPosition += 9;
        addLeftRightText("📅 Order Date:", new Date(order.CreatedAt).toLocaleDateString(), yPosition);
        yPosition += 9;
        addLeftRightText("📊 Status:", getStatusText(order.StatusID).toUpperCase(), yPosition);
        yPosition += 9;
        ...
        if (order.Details && order.Details.length > 0) {
            ...
            order.Details.forEach((detail, index) => {
                ...
                const productName = detail.ProductName.length > 25 ?
                    detail.ProductName.substring(0, 22) + "..." : detail.ProductName;
            ...
            doc.setFont("helvetica", "bold");
            doc.setFontSize(14);
            doc.text(`TOTAL: $${order.FinalTotal.toFixed(2)}`, 155, yPosition + 10, { align: "center" });
            yPosition += 25;
        }
        ...
        doc.setFont("helvetica", "normal");
        doc.setFontSize(10);
        addCenteredText("🎉 Thank you for choosing Smart Wine Store!", 10, footerY + 5, [255, 255, 255]);
        addCenteredText("For questions about this order, please contact us at info@smartwinestore.com", 8, footerY + 12, [255, 255, 255]);
        addCenteredText(`Generated on ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}`, 7, footerY + 18, [255, 255, 255]);
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {currentOrders.length === 0 ? (
                                <tr>
                                    ...
                                    >
                                        {searchTerm ? t("admin.orders.noOrdersSearch") : t("admin.orders.noOrders")}
                                    </td>
                                </tr>
                            ) : (
