...
export default function DiscountProductsPage() {
  ...
  const [selectedProduct, setSelectedProduct] =
    useState<DiscountProduct | null>(null);
  ...
  const handleDelete = (id: number) => {
    ...
    if (product) {
      ...
      if (status === "Active") {
        alert(
          "Cannot delete active product discounts! Please wait until the discount expires or edit the end date."
        );
        return;
      }
  ...
  const handleConfirmDelete = () => {
    if (productToDelete) {
      ...
      alert(
        `Product discount "${productToDelete.ProductName}" deleted successfully!`
      );
  ...
  const handleUpdateProduct = async (
    ...
    try {
      const response = await discountProductsRepository.updateDiscountProduct(
        id,
        {
          DiscountID: (data as any).DiscountID,
          ProductID: (data as any).ProductID,
