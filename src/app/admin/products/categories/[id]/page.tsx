...
export default function CategoryDetailPage() {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        ...
        const staticCategory = productCategories.find(
  ...
  useEffect(() => {
    const fetchCategoryProducts = async () => {
      try {
        const response =
          await categoriesRepository.getProductsByCategory(categoryId);
  ...
  const handleSaveConfirm = async () => {
    ...
    try {
      const updateData = {
        ...
        CategoryParentID:
          category.CategoryParentID === 0 || !category.CategoryParentID
            ? null
  ...
  return (
    <div>
      <PageHeader title="Category Detail" icon={Wine} iconColor="text-black" />
                      ...
                      <tbody className="divide-y divide-gray-200 dark:divide-slate-600">
                        {categoryProducts.map((product, index) => (
                          <tr
                            ...
                            className="hover:bg-gray-50 dark:hover:bg-slate-600/50"
                          >
